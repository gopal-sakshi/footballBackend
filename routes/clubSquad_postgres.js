...
addPlayerCallback = async function (req, res) {
    ...
    const params12 = [ 
        req.body.id,  req.body.player, req.body.position, req.body.country, 
        req.body.comments,  req.body.otherclubs, req.body.isactive, req.body.year_joined 
    ];
    pgPool.connect((err, client, done) => {
        if(err) res.send(err);
...
router.get('/inCaps', inCapsCb);                                // pgPool callback; returnCaps UDF
router.post('/transferFunds', transferFundsCb );                // pgPool callback; rm_transfer SP
router.get('/canteen', getCanteen);                             // pgPool callback
router.post('/:clubName/addPlayer', addPlayerCallback);         // pgPool callback; using params
router.get('/:clubName', squadsCallback);                       // pgPool callback & promise; pgClient
/*********************************************** */
